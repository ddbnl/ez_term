...
impl RadioButtonState {
    pub fn new(path: String, scheduler: &mut SchedulerFrontend) -> Self {
        RadioButtonState {
            path: path.clone(),
            group: scheduler.new_string_property(format!("{}/group", path).as_str(), String::new()),
            ...
            active: scheduler.new_bool_property(format!("{}/active", path).as_str(), false),
            active_symbol: scheduler
                .new_string_property(format!("{}/active_symbol", path).as_str(), "X".to_string()),
            inactive_symbol: scheduler.new_string_property(
                format!("{}/inactive_symbol", path).as_str(),
                "-".to_string(),
            ),
...
impl GenericState for RadioButtonState {
    ...
    fn get_property(&self, name: &str) -> EzValues {
        match name {
            ...
            "group" => EzValues::String(self.group.value.to_string()),
            "active" => EzValues::Bool(self.active.value),
            "active_symbol" => EzValues::String(self.active_symbol.value.to_string()),
            "inactive_symbol" => EzValues::String(self.inactive_symbol.value.to_string()),
    ...
    fn copy_state_values(&mut self, other: EzState) {
        ...
        self.group.copy_from(&other.group);
        self.active.copy_from(&other.active);
        self.active_symbol.copy_from(&other.active_symbol);
        self.inactive_symbol.copy_from(&other.inactive_symbol);
    }
...
impl RadioButtonState {
    ...
    pub fn set_group(&mut self, group: &str) {
        self.group.set(group.to_string());
    }
    ...
    pub fn set_active_symbol(&mut self, symbol: &str) {
        self.active_symbol.set(symbol.to_string());
    }

    pub fn get_active_symbol(&self) -> String {
        self.active_symbol.value.clone()
    }

    pub fn set_inactive_symbol(&mut self, symbol: &str) {
        self.inactive_symbol.set(symbol.to_string());
    }

    pub fn get_inactive_symbol(&self) -> String {
        self.inactive_symbol.value.clone()
    }
}
