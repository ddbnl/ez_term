...
impl EzProperties {
    pub fn get_generic_value(&self) -> EzValues {
        if let EzProperties::Usize(i) = self {
            EzValues::Usize(i.value)
        } else if let EzProperties::F64(i) = self {
            EzValues::F64(i.value)
        } else if let EzProperties::Bool(i) = self {
            EzValues::Bool(i.value)
        } else if let EzProperties::String(i) = self {
            EzValues::String(i.value.clone())
        } else if let EzProperties::Color(i) = self {
            EzValues::Color(i.value)
        } else if let EzProperties::LayoutMode(i) = self {
            EzValues::LayoutMode(i.value.clone())
        } else if let EzProperties::LayoutOrientation(i) = self {
            EzValues::LayoutOrientation(i.value.clone())
        } else if let EzProperties::VerticalAlignment(i) = self {
            EzValues::VerticalAlignment(i.value)
        } else if let EzProperties::HorizontalAlignment(i) = self {
            EzValues::HorizontalAlignment(i.value)
        } else if let EzProperties::HorizontalPosHint(i) = self {
            EzValues::HorizontalPosHint(i.value)
        } else if let EzProperties::VerticalPosHint(i) = self {
            EzValues::VerticalPosHint(i.value)
        } else if let EzProperties::SizeHint(i) = self {
            EzValues::SizeHint(i.value)
        } else {
            panic!("Unimplemented variant")
        }
    }
    ...
    pub fn as_f64(&self) -> &EzProperty<f64> {
        if let EzProperties::F64(i) = self {
            i
        } else {
    ...
    pub fn as_bool(&self) -> &EzProperty<bool> {
        if let EzProperties::Bool(i) = self {
            i
        } else {
    ...
    pub fn as_color(&self) -> &EzProperty<Color> {
        if let EzProperties::Color(i) = self {
            i
        } else {
    ...
    pub fn as_layout_mode(&self) -> &EzProperty<LayoutMode> {
        if let EzProperties::LayoutMode(i) = self {
            i
        } else {
    ...
    pub fn as_layout_orientation(&self) -> &EzProperty<LayoutOrientation> {
        if let EzProperties::LayoutOrientation(i) = self {
            i
        } else {
    ...
    pub fn as_vertical_alignment(&self) -> &EzProperty<VerticalAlignment> {
        if let EzProperties::VerticalAlignment(i) = self {
            i
        } else {
    ...
    pub fn as_horizontal_alignment(&self) -> &EzProperty<HorizontalAlignment> {
        if let EzProperties::HorizontalAlignment(i) = self {
            i
        } else {
    ...
    pub fn as_horizontal_pos_hint(&self) -> &EzProperty<HorizontalPosHint> {
        if let EzProperties::HorizontalPosHint(i) = self {
            i
        } else {
    ...
    /// Get a mutable ref [EzProperty<HorizontalPosHint>] from this enum. You must be sure this
    /// is a HorizontalPosHint property or it will panic.
    pub fn as_horizontal_pos_hint_mut(&mut self) -> &mut EzProperty<HorizontalPosHint> {
        if let EzProperties::HorizontalPosHint(i) = self {
            i
        } else {
            panic!("Wrong property, expected HorizontalPosHintProperty")
    ...
    pub fn as_vertical_pos_hint(&self) -> &EzProperty<VerticalPosHint> {
        if let EzProperties::VerticalPosHint(i) = self {
            i
        } else {
    ...
    /// Get a mutable ref [EzProperty<VerticalPosHint>] from this enum. You must be sure this
    /// is a VerticalPosHint property or it will panic.
    pub fn as_vertical_pos_hint_mut(&mut self) -> &mut EzProperty<VerticalPosHint> {
        if let EzProperties::VerticalPosHint(i) = self {
            i
        } else {
            panic!("Wrong property, expected VerticalPosHintProperty")
    ...
    pub fn as_size_hint(&self) -> &EzProperty<Option<f64>> {
        if let EzProperties::SizeHint(i) = self {
            i
        } else {
