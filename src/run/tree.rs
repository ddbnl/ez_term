...
impl<T> Tree<T> {
    ...
    fn _add_node(&mut self, mut steps: Vec<&str>, node: T) {
        ...
        if steps.len() == 1 {
            ...
            self.objects
                .get_mut(id)
                .unwrap_or_else(|| panic!("{} not found in {}", id, self.id))
    ...
    pub fn remove_node(&mut self, path: String) -> Self {
        let steps = if !path.contains('/') {
            ...
            if _steps[0] == self.id {
                panic!(
                    "Cannot remove self from tree. Remove from parent \
                                           node instead"
                )
            }
            _steps
    ...
    fn _remove_node(&mut self, mut steps: Vec<&str>) -> Self {
        if steps.len() == 1 {
            ...
            self.objects.remove(id).unwrap_or_else(|| {
                panic!(
                    "Node '{}' could not resolve '{:?}' at step '{}' when removing state",
                    self.id, steps, id
                )
            })
        } else {
...
impl Tree<EzState> {
    ...
    pub fn as_layout(&self) -> &LayoutState {
        if let EzState::Layout(ref i) = self.obj {
            i
        } else {
            panic!("LayoutState is the wrong type for {}.", self.id)
        }
    }

    /// Cast this state as a mutable layout state ref, you must be sure you have one.
    pub fn as_layout_mut(&mut self) -> &mut LayoutState {
        if let EzState::Layout(ref mut i) = self.obj {
            i
        } else {
            panic!("LayoutState is the wrong type for {}.", self.id)
        }
    ...
    pub fn as_canvas(&self) -> &CanvasState {
        if let EzState::Canvas(ref i) = self.obj {
            i
        } else {
            panic!("CanvasState is the wrong type for {}.", self.id)
        }
    }
    ...
    pub fn as_canvas_mut(&mut self) -> &mut CanvasState {
        if let EzState::Canvas(ref mut i) = self.obj {
            i
        } else {
            panic!("CanvasState is the wrong type for {}.", self.id)
        }
    ...
    pub fn as_label(&self) -> &LabelState {
        if let EzState::Label(ref i) = self.obj {
            i
        } else {
            panic!("LabelState is the wrong type for {}.", self.id)
        }
    }
    ...
    pub fn as_label_mut(&mut self) -> &mut LabelState {
        if let EzState::Label(ref mut i) = self.obj {
            i
        } else {
            panic!("LabelState is the wrong type for {}.", self.id)
        }
    ...
    pub fn as_button(&self) -> &ButtonState {
        if let EzState::Button(ref i) = self.obj {
            i
        } else {
            panic!("ButtonState is the wrong type for {}.", self.id)
        }
    }
    ...
    pub fn as_button_mut(&mut self) -> &mut ButtonState {
        if let EzState::Button(ref mut i) = self.obj {
            i
        } else {
            panic!("ButtonState is the wrong type for {}.", self.id)
        }
    ...
    pub fn as_slider(&self) -> &SliderState {
        if let EzState::Slider(ref i) = self.obj {
            i
        } else {
            panic!("SliderState is the wrong type for {}.", self.id)
        }
    }
    ...
    pub fn as_slider_mut(&mut self) -> &mut SliderState {
        if let EzState::Slider(ref mut i) = self.obj {
            i
        } else {
            panic!("SliderState is the wrong type for {}.", self.id)
        }
    ...
    pub fn as_progress_bar(&self) -> &ProgressBarState {
        if let EzState::ProgressBar(ref i) = self.obj {
            i
        } else {
            panic!("ProgressBarState is the wrong type for {}.", self.id)
        }
    }
    ...
    pub fn as_progress_bar_mut(&mut self) -> &mut ProgressBarState {
        if let EzState::ProgressBar(ref mut i) = self.obj {
            i
        } else {
            panic!("ProgressBarState is the wrong type for {}.", self.id)
        }
    ...
    pub fn as_checkbox(&self) -> &CheckboxState {
        if let EzState::Checkbox(ref i) = self.obj {
            i
        } else {
            panic!("CheckboxState is the wrong type for {}.", self.id)
        }
    }
    ...
    pub fn as_checkbox_mut(&mut self) -> &mut CheckboxState {
        if let EzState::Checkbox(ref mut i) = self.obj {
            i
        } else {
            panic!("CheckboxState is the wrong type for {}.", self.id)
        }
    ...
    pub fn as_dropdown(&self) -> &DropdownState {
        if let EzState::Dropdown(ref i) = self.obj {
            i
        } else {
            panic!("DropdownState is the wrong type for {}.", self.id)
        }
    }
    ...
    pub fn as_dropdown_mut(&mut self) -> &mut DropdownState {
        if let EzState::Dropdown(ref mut i) = self.obj {
            i
        } else {
            panic!("DropdownState is the wrong type for {}.", self.id)
        }
    ...
    pub fn as_dropped_down_menu(&self) -> &DroppedDownMenuState {
        if let EzState::DroppedDownMenu(ref i) = self.obj {
            i
        } else {
            panic!("DroppedDownMenuState is the wrong type for {}.", self.id)
        }
    }
    ...
    pub fn as_dropped_down_menu_mut(&mut self) -> &mut DroppedDownMenuState {
        if let EzState::DroppedDownMenu(ref mut i) = self.obj {
            i
        } else {
            panic!("DroppedDownMenuState is the wrong type for {}.", self.id)
        }
    ...
    pub fn as_radio_button(&self) -> &RadioButtonState {
        if let EzState::RadioButton(ref i) = self.obj {
            i
        } else {
            panic!("RadioButtonState is the wrong type for {}.", self.id)
        }
    }
    ...
    pub fn as_radio_button_mut(&mut self) -> &mut RadioButtonState {
        if let EzState::RadioButton(ref mut i) = self.obj {
            i
        } else {
            panic!("RadioButtonState is the wrong type for {}.", self.id)
        }
    ...
    pub fn as_text_input(&self) -> &TextInputState {
        if let EzState::TextInput(ref i) = self.obj {
            i
        } else {
            panic!("TextInputState is the wrong type for {}.", self.id)
        }
    }
    ...
    pub fn as_text_input_mut(&mut self) -> &mut TextInputState {
        if let EzState::TextInput(ref mut i) = self.obj {
            i
        } else {
            panic!("TextInputState is the wrong type for {}.", self.id)
        }
...
pub fn initialize_state_tree(root_layout: &Layout) -> StateTree {
    let mut state_tree = StateTree::new("root".to_string(), root_layout.get_state());
    for child in root_layout.get_widgets_recursive() {
        state_tree.add_node(
            child.as_ez_object().get_path(),
...
pub fn initialize_callback_tree(root_layout: &Layout) -> CallbackTree {
    let mut callback_tree = CallbackTree::new("root".to_string(), CallbackConfig::default());
    for child in root_layout.get_widgets_recursive() {
...
pub fn clean_trees(
    ...
    let state_paths: Vec<String> = state_tree
        .get_all()
        .iter()
        .map(|x| x.as_generic().get_path().clone())
        .collect();
