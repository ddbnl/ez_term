...
use crate::widgets::helper_functions::{
    add_border, add_padding, reposition_with_pos_hint, resize_with_size_hint,
};
use crate::Context;
...
impl EzObject for Layout {
    fn load_ez_parameter(
        ...
        match parameter_name.as_str() {
                ...
                let (x, y) = match parameter_value.split_once(',') {
                    ...
                    None => {
                        return Err(Error::new(
                            ...
                            format!(
                                "Invalid value for scroll: \"{}\". Required format \
                                   is \"scroll: true, false\"",
                                parameter_value
                            ),
                        ))
                    }
            ...
            _ => {
                return Err(Error::new(
                    ErrorKind::InvalidData,
                    format!("Invalid parameter name for layout {}", parameter_name),
                ))
            }
        }
        Ok(())
    ...
    fn on_left_mouse_click(
        ...
        if state.get_scrolling_config().get_is_scrolling_y() && mouse_pos.x == h_edge {
    ...
    fn on_drag(
        ...
        let offset = if state.get_border_config().get_border() {
            2
        } else {
            1
        };
            ...
            && mouse_pos.x + offset as isize == state.get_size().get_width() as isize
        {
            ...
            let root_size = state_tree.as_generic().get_size().clone();
            let state = state_tree.get_mut(&self.path).as_layout_mut();
            state.set_x(if diff_x < 0 && diff_x.abs() > pos.x.value as isize {
                0
            } else if diff_x > 0
                && pos.x.value + diff_x as usize > (root_size.get_width() - size.get_width())
            {
                root_size.get_width() - size.get_width()
            } else {
                (pos.x.value as isize + diff_x) as usize
            });
            state.set_y(if diff_y < 0 && diff_y.abs() > pos.y.value as isize {
                0
            } else if diff_y > 0
                && pos.y.value + diff_y as usize > (root_size.get_height() - size.get_height())
            {
    ...
    fn get_clone(&self, scheduler: &mut SchedulerFrontend) -> EzObjects {
        let children: Vec<EzObjects> = self
            .children
            .iter()
            .map(|x| x.as_ez_object().get_clone(scheduler))
            .collect();
...
impl Layout {
    ...
    fn handle_scroll_drag_x(
        ...
        let absolute_diff = (mouse_pos.x as isize).abs_diff(previous_pos.unwrap().x as isize);
        let absolute_scroll = 1.0 / ((absolute_diff * state.get_effective_size().width) as f64);
        if previous_pos.unwrap().x > mouse_pos.x
            && absolute_scroll > state.get_scrolling_config().get_scroll_start_x()
        {
    ...
    fn handle_scroll_drag_y(
        ...
        let absolute_diff = (mouse_pos.y as isize).abs_diff(previous_pos.unwrap().y as isize);
    ...
    fn auto_scale_to_content(&self, state_tree: &mut StateTree, contents: PixelMap) -> PixelMap {
        ...
        if state.get_auto_scale().get_auto_scale_height() {
            let auto_scale_height = contents.iter().map(|x| x.len()).max().unwrap_or(0);
    ...
    fn get_modal_contents(&self, state_tree: &mut StateTree, mut contents: PixelMap) -> PixelMap {
        ...
        // Size modal
        let parent_size = state_tree
            .get(&self.get_path())
            .as_layout()
            .get_size()
            .clone();
        let modal = state_tree
            .get(&self.get_path())
            .as_layout()
            .get_modal()
            .clone();
        let state = &mut state_tree.get_mut(&modal.as_ez_object().get_path()).obj;
        resize_with_size_hint(state, parent_size.get_width(), parent_size.get_height());
        reposition_with_pos_hint(
            parent_size.get_width(),
        ...
        state
            .as_generic_mut()
            .set_absolute_position(IsizeCoordinates::new(x as isize, y as isize));
        ...
        let state = state_tree
            .get_mut(&modal.as_ez_object().get_path())
            .as_generic();
        let start_pos = state.get_position();
        for x in 0..modal_content.len() {
            for y in 0..modal_content[x].len() {
                let write_pos = Coordinates::new(start_pos.get_x() + x, start_pos.get_y() + y);
                if write_pos.x < parent_size.get_width() && write_pos.y < parent_size.get_height() {
    ...
    pub fn add_user_filler(&self, state_tree: &mut StateTree, mut contents: PixelMap) -> PixelMap {
        ...
        let filler = Pixel::new(
            state.get_filler_symbol(),
    ...
    pub fn get_children_in_view(&self, state_tree: &mut StateTree) -> &[EzObjects] {
        ...
        let state = state_tree.get_mut(&self.path).as_layout_mut();
        if state.get_view_size() == 0 {
            return &self.children;
        }
        if state.get_view_page() == 0 {
            state.set_view_page(1)
        }

        let max_pages = (self.children.len() / state.get_view_size())
            + if self.children.len() % state.get_view_size() > 0 {
                1
            } else {
                0
            };
        if state.get_view_page() > max_pages {
            state.set_view_page(max_pages)
        }
