...
impl Layout {
    ...
    pub fn get_box_mode_contents(&self, state_tree: &mut StateTree) -> PixelMap {
        match state_tree.get(&self.path).as_layout().get_orientation() {
            ...
            _ => panic!(
                "Error in layout: {}, mode \"Box\" requires orientation \
    ...
    /// own [height] for each.
    pub fn get_box_mode_horizontal_orientation_contents(
        &self,
        ...
        let (filler_symbol, filler_fg_color, filler_bg_color) = if state.get_fill() {
            (
                state.get_filler_symbol(),
                own_colors.get_filler_fg_color(),
        ...
        for child in self.get_children_in_view(state_tree) {
            ...
            let width_left = if !own_scrolling.get_scroll_x()
                && !own_infinite_size.width
                && !state.get_infinite_size().width
                && own_effective_size.width >= position.x
            {
                own_effective_size.width - position.x
            } else {
                0
            ...
            // panicking.
            if !own_scrolling.get_scroll_x()
                && !own_infinite_size.width
                && state.get_size().get_width() > width_left
            {
                state.get_size_mut().set_width(width_left);
            }
            if !own_scrolling.get_scroll_y()
        ...
        let own_effective_size = state_tree
        ...
        for (i, content) in content_list.into_iter().enumerate() {
            merged_content = self.merge_horizontal_contents(
                ...
                own_infinite_size.height,
                state_tree
                    .get_mut(&self.children.get(i).unwrap().as_ez_object().get_path())
                    .as_generic_mut(),
                filler_symbol.clone(),
                filler_fg_color,
                filler_bg_color,
    ...
    pub fn get_box_mode_vertical_orientation_contents(
        &self,
        state_tree: &mut StateTree,
    ) -> PixelMap {
        ...
        for child in self.get_children_in_view(state_tree) {
            ...
            let height_left = if !own_scrolling.get_scroll_y()
                && !own_infinite_size.height
            ...
            if child_state.get_auto_scale().get_auto_scale_width() {
                child_state
                    .get_size_mut()
                    .set_width(own_effective_size.width)
            }
            ...
            if state.get_infinite_size().height {
                state
                    .get_size_mut()
                    .set_height(if !child_content.is_empty() {
                        child_content[0].len()
                    } else {
                        0
                    })
            }
            position.y += if !child_content.is_empty() {
                child_content[0].len()
            } else {
                0
            };
    ...
    pub fn merge_horizontal_contents(
        &self,
        mut merged_content: PixelMap,
        mut new: PixelMap,
        parent_height: usize,
        parent_infinite_height: bool,
    ...
    pub fn merge_vertical_contents(
        ...
        let fill_pixel = Pixel::new(filler_symbol, fill_fg_color, fill_bg_color);
