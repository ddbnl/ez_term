...
impl Layout {
    ...
    pub fn set_child_sizes(&self, state_tree: &mut StateTree) {
        let own_state = state_tree.get_mut(&self.get_path()).as_layout();
                ...
                if own_mode == LayoutMode::Box {
                    ...
                    let divide_by = if own_cols > 0 {
                        ...
                        self.children.len() / own_rows
                            + if self.children.len() % own_rows > 0 {
                                1
                            } else {
                                0
                            }
                ...
                if own_mode == LayoutMode::Box {
                    ...
                    let divide_by = if own_rows > 0 {
                        ...
                        self.children.len() / own_cols
                            + if self.children.len() % own_cols > 0 {
                                1
                            } else {
                                0
                            }
    ...
    fn check_default_size_hints(&self, state_tree: &StateTree) -> (bool, bool) {
        ...
        let own_orientation = state_tree
            .get(&self.path)
            .as_layout()
            .get_orientation()
            .clone();
    ...
    pub fn propagate_absolute_positions(&self, state_tree: &mut StateTree) {
        let absolute_position = state_tree
            .get(&self.path)
            .as_generic()
            .get_effective_absolute_position();
        let effective_size = state_tree
            .get(&self.path)
            .as_layout()
            .get_effective_size()
            .clone();
        let scrolling = state_tree
            .get(&self.path)
            .as_layout()
        ...
        for child in self.get_children() {
            if let EzObjects::Layout(i) = child {
                let child_state = state_tree.get_mut(&i.get_path()).as_generic_mut();
                ...
                let child_state = state_tree
                    .get_mut(&child.as_ez_object().get_path())
                    .as_generic_mut();
                let pos = child_state.get_position();
                let mut new_absolute_position = IsizeCoordinates::new(
                    absolute_position.x + pos.get_x() as isize,
        ...
        if self.state.get_mode() == &LayoutMode::Tab {
            if let EzObjects::Layout(_) = child.clone() {
                ...
                tab_header
                    ...
                    .colors
                    .fg_color
                    .set(self.state.colors.tab_header_fg_color.value);
                tab_header
                    .state
                    .colors
                    .bg_color
                    .set(self.state.colors.tab_header_bg_color.value);
                tab_header
                    .state
                    .colors
                    .disabled_fg_color
                    .set(self.state.colors.tab_header_active_fg_color.value);
                tab_header
                    .state
                    .colors
                    .disabled_bg_color
                    .set(self.state.colors.tab_header_active_bg_color.value);
                tab_header
                    .state
                    .colors
                    .border_fg_color
                    .set(self.state.colors.tab_header_border_fg_color.value);
                tab_header
                    .state
                    .colors
                    .border_bg_color
                    .set(self.state.colors.tab_header_border_bg_color.value);
                tab_header
                    .state
                    .colors
                    .selection_fg_color
                    .set(self.state.colors.selection_fg_color.value);
                tab_header
                    .state
                    .colors
                    .selection_bg_color
                    .set(self.state.colors.selection_bg_color.value);

                let tab_on_click = move |context: Context| {
                    let state = context.state_tree.get_mut(&parent_path).as_layout_mut();
                    state.set_active_tab(&tab_path.clone());
                    state.update(context.scheduler);
                    true
                };
                let callback_config = CallbackConfig::from_on_press(Box::new(tab_on_click));
    ...
    pub fn remove_child(&mut self, id: &str) {
        let widget_index = self
            .child_lookup
            .get(id)
            .unwrap_or_else(|| panic!("Could not remove widget: {}. It could not be found.", id))
            .clone();
    ...
    pub fn get_children(&self) -> &Vec<EzObjects> {
        &self.children
    }

    /// Get a mutable list of children non-recursively. Can be [layout] or [EzWidget]
    pub fn get_children_mut(&mut self) -> &mut Vec<EzObjects> {
        &mut self.children
    }
    ...
    pub fn get_child_by_path(&self, path: &str) -> Option<&EzObjects> {
        ...
        let mut root = if first == "modal" {
            if self.state.has_modal() {
                self.state.get_modal()
            } else {
                return None;
            }
        } else {
            if let Some(i) = self.get_child(first) {
                i
            } else {
                return None;
            }
    ...
    pub fn get_child_by_path_mut(&mut self, path: &str) -> Option<&mut EzObjects> {
        ...
        let mut root = if first == "modal" {
            if self.state.has_modal() {
                self.state.get_modal_mut()
            } else {
                return None;
            }
        } else {
            if let Some(i) = self.get_child_mut(first) {
                i
            } else {
                return None;
            }
    ...
    pub fn scale_to_largest_child(&self, content_list: &[PixelMap], state_tree: &mut StateTree) {
        ...
        if state.get_auto_scale().get_auto_scale_width() {
            state.set_effective_width(if state.get_orientation() == &LayoutOrientation::Vertical {
        ...
        if state.get_auto_scale().get_auto_scale_height() {
            state.set_effective_height(
                if state.get_orientation() == &LayoutOrientation::Horizontal {
                    content_list
                        .iter()
                        .map(|child| child.iter().map(|x| x.len()).max().unwrap_or(0))
                        .max()
                        .unwrap_or(0)
                } else {
                    content_list
