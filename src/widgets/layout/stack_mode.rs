...
use crate::states::definitions::{
    AutoScale, ColorConfig, InfiniteSize, LayoutOrientation, ScrollingConfig,
};
...
impl Layout {
    ...
    fn get_orientated_content(
        ...
        match orientation {
            ...
            LayoutOrientation::LeftRightBottomTop => self.get_left_right_bottom_top_content(
                state_tree,
                content_list,
                effective_size,
                colors,
            ...
            LayoutOrientation::RightLeftTopBottom => self.get_right_left_top_bottom_content(
                state_tree,
                content_list,
                effective_size,
                colors,
            ...
            LayoutOrientation::RightLeftBottomTop => self.get_right_left_bottom_top_content(
                state_tree,
                content_list,
                effective_size,
                colors,
            ...
            LayoutOrientation::TopBottomLeftRight => self.top_bottom_left_right_content(
                state_tree,
                content_list,
                effective_size,
                colors,
            ...
            LayoutOrientation::TopBottomRightLeft => self.top_bottom_right_left_content(
                state_tree,
                content_list,
                effective_size,
                colors,
            ...
            LayoutOrientation::BottomTopLeftRight => self.bottom_top_left_right_content(
                state_tree,
                content_list,
                effective_size,
                colors,
            ...
            LayoutOrientation::BottomTopRightLeft => self.bottom_top_right_left_content(
                state_tree,
                content_list,
                effective_size,
                colors,
    ...
    fn get_stack_mode_child_content(
        &self,
        state_tree: &mut StateTree,
    ...
    fn get_left_right_top_bottom_content(
        ...
        for (i, content) in content_list.iter().enumerate() {
            ...
            let state = state_tree
                .get_mut(&self.children[i].as_ez_object().get_path())
                .as_generic_mut();
            state.set_x(pos.x);
            state.set_y(pos.y);
            for child_x in 0..content.len() {
                for child_y in 0..content[child_x].len() {
                    ...
                    if write_pos_y >= effective_size.height {
                        continue;
                    }
                    if write_pos_x > largest_x {
                        largest_x = pos.x + child_x
                    }
                    if write_pos_y > largest_y {
                        largest_y = pos.y + child_y
                    }
    ...
    fn get_left_right_bottom_top_content(
        ...
        for (i, content) in content_list.iter().enumerate() {
            ...
            for child_x in 0..content.len() {
                for child_y in 0..content[child_x].len() {
                    if pos.y + child_y > effective_size.height {
                        continue;
                    }
                    if pos.x + child_x > largest_x {
                        largest_x = pos.x + child_x
                    }
                    if pos.y + child_y < smallest_y {
                        smallest_y = pos.y + child_y
                    }
                    merged_content[pos.x + child_x][pos.y + child_y] =
                        content[child_x][child_y].clone();
                }
    ...
    fn get_right_left_top_bottom_content(
        ...
        for (i, content) in content_list.iter().enumerate() {
            ...
            if content.len() > pos.x {
                let largest = content.iter().map(|x| x.len()).max().unwrap();
                if pos.y + largest >= effective_size.height {
                    break;
                }
                pos.y = largest_y + 1;
                let largest = content.iter().map(|x| x.len()).max().unwrap();
            ...
            for child_x in 0..content.len() {
                for child_y in 0..content[child_x].len() {
                    ...
                    if pos.x + child_x < smallest_x {
                        smallest_x = pos.x + child_x
                    }
                    if pos.y + child_y > largest_y {
                        largest_y = pos.y + child_y
                    }
                    merged_content[pos.x + child_x][pos.y + child_y] =
                        content[child_x][child_y].clone();
    ...
    fn get_right_left_bottom_top_content(
        ...
        let mut pos = Coordinates::new(effective_size.width - 1, effective_size.height - 1);
        for (i, content) in content_list.iter().enumerate() {
            if content.len() > effective_size.width + 1 {
                break;
            }
    ...
    fn top_bottom_left_right_content(
        ...
        for (i, content) in content_list.iter().enumerate() {
            let largest = content.iter().map(|x| x.len()).max().unwrap();
            if largest > effective_size.height + 1 {
                break;
            }
            if largest > effective_size.height - pos.y {
                if pos.x + largest_x >= effective_size.width {
                    break;
                }
                pos.x = largest_x + 1;
                if content.len() > effective_size.width - pos.x {
    ...
    fn top_bottom_right_left_content(
        ...
        for (i, content) in content_list.iter().enumerate() {
            let largest = content.iter().map(|x| x.len()).max().unwrap();
            if largest > effective_size.height {
                break;
            }
            ...
            for child_x in 0..content.len() {
                for child_y in 0..content[child_x].len() {
                    if pos.x + child_x >= effective_size.width
                        || pos.y + child_y >= effective_size.height
                    {
                        continue;
                    }
                    if pos.x + child_x < smallest_x {
    ...
    fn get_filler_content(
        ...
        let merged_content =
            vec![
                vec!(Pixel::new(symbol, fg_color, bg_color); effective_size.height);
