...
impl EzObject for ProgressBar {
        ...
        match parameter_name.as_str() {
            ...
            _ => {
                return Err(Error::new(
                    ...
                    format!(
                        "Invalid parameter name for progress bar: {}",
                        parameter_name
                    ),
                ))
            }
    ...
    fn get_state(&self) -> EzState {
        EzState::ProgressBar(self.state.clone())
    }
    ...
    fn get_contents(&self, state_tree: &mut StateTree) -> PixelMap {
        let state = state_tree.get_mut(&self.get_path()).as_progress_bar_mut();
        state.set_height(1);
        ...
        let value_pos =
            ((state.get_effective_size().width - 1) as f64 * state.get_normalized_value()) as usize;
        ...
        let state = state_tree.get(&self.get_path()).as_progress_bar();
