    ...
    match property_name {
        ...
        "size_hint" => {
            let (x,y) = match property_value.trim().split_once(',') {
                Some((i, j)) => (i, j),
                None => return Err(Error::new(ErrorKind::InvalidData,
                                              format!("Could not load pos parameter: \"{}\". \
                                              It must be in the form \"size_hint: 0.3, 0.3\"", property_value)))
            };
            load_base_properties::load_size_hint_property(
                x.trim(), scheduler, path.clone(), "size_hint_x", state)?;
            load_base_properties::load_size_hint_property(
                y.trim(), scheduler, path, "size_hint_y", state)?;
        }
        ...
        "pos_hint" => {
            ...
            load_base_properties::load_horizontal_pos_hint_property(
                x_str, scheduler, path.clone(), "pos_hint_x", state)?;
            load_base_properties::load_vertical_pos_hint_property(
                y_str, scheduler, path, "pos_hint_y", state)?;
        },
        ...
        bool values. If you want to set only one, use auto_scale_height or auto_scale_width instead."));
            load_base_properties::load_bool_property(
                width_str.trim(), scheduler, path.clone(), "auto_scale_width", state)?;
            load_base_properties::load_bool_property(
                height_str.trim(), scheduler, path, "auto_scale_height", state)?;
        },
        ...
        "padding" => {
            ...
            load_base_properties::load_usize_property(
                padding_params[0], scheduler, path.clone(), "padding_top", state)?;
            load_base_properties::load_usize_property(
                padding_params[1], scheduler, path.clone(), "padding_bottom", state)?;
            load_base_properties::load_usize_property(
                padding_params[2], scheduler, path.clone(), "padding_left", state)?;
            load_base_properties::load_usize_property(
                padding_params[3], scheduler, path.clone(), "padding_right", state)?;
        },
        "padding_x" => {
            ...
            load_base_properties::load_usize_property(
                left.trim(), scheduler, path.clone(), "padding_left", state)?;
            load_base_properties::load_usize_property(
                right.trim(), scheduler, path.clone(), "padding_right", state)?;
        },
        "padding_y" => {
            ...
            load_base_properties::load_usize_property(
                top.trim(), scheduler, path.clone(), "padding_top", state)?;
            load_base_properties::load_usize_property(
                bottom.trim(), scheduler, path.clone(), "padding_bottom", state)?;
        },
